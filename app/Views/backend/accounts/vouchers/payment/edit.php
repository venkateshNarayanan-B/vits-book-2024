        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card card-info">
                    ...
                    <?= form_open('payment-vouchers/update/' . $voucher['id'], ['class' => 'form-horizontal']) ?>
                    <div class="card-body">
                        <!-- Date -->
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <?= form_input([
                                    'name' => 'date',
                                    'value' => old('date', date('Y-m-d', strtotime($voucher['date']))), // Format the date to YYYY-MM-DD
                                    ...
                                    <?php foreach ($entries as $index => $entry): ?>
                                        <div class="entry mb-3">
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-9">
                                                    <select name="entries[<?= $index ?>][ledger_id]" class="form-control select2">
                                                        <option value="">Select Ledger</option>
                                                        <?php foreach ($ledgers as $ledger): ?>
                                                            <option value="<?= esc($ledger['id']) ?>"
                                                                <?= $entry['ledger_id'] == $ledger['id'] ? 'selected' : '' ?>>
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-9">
                                                    <input type="number" name="entries[<?= $index ?>][credit]"
                                                        class="form-control" placeholder="Credit" step="0.01"
                                                        value="<?= esc($entry['credit']) ?>" required>
                                                </div>
                                            </div>
                                        </div>
                                    <?php endforeach; ?>
                    ...
                    <div class="card-footer">
                        <?= form_submit('submit', 'Update Payment Voucher', ['class' => 'btn btn-info']) ?>
                        <a href="<?= site_url('payment-vouchers') ?>" class="btn btn-default float-right">Cancel</a>
                    </div>
                    <?= form_close() ?>
                </div>
            </div>
            ...
            const entryDiv = `
                <div class="entry mb-3">
                    ...
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label">Credit</label>
                        <div class="col-sm-9">
                            <input type="number" name="entries[${entryCount}][credit]" class="form-control" placeholder="Credit" step="0.01" required>
                        </div>
                    </div>
                </div>`;
