        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card card-info">
                    ...
                    <?= form_open('contra-vouchers/update/' . $voucher['id'], ['class' => 'form-horizontal']) ?>
                    <div class="card-body">
                        <!-- Date -->
                            ...
                            <div class="col-sm-9">
                                <select name="debit_ledger" class="form-control select2" required>
                                    ...
                                    <?php foreach ($ledgers as $ledger): ?>
                                        <option value="<?= esc($ledger['id']) ?>" <?= old('debit_ledger', $debit_entry['ledger_id'] ?? '') == $ledger['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select name="credit_ledger" class="form-control select2" required>
                                    ...
                                    <?php foreach ($ledgers as $ledger): ?>
                                        <option value="<?= esc($ledger['id']) ?>" <?= old('credit_ledger', $credit_entry['ledger_id'] ?? '') == $ledger['id'] ? 'selected' : '' ?>>
                                            <?= esc($ledger['ledger_name']) ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <?= form_input([
                                    'name' => 'amount',
                                    'value' => old('amount', $amount), // Display the amount fetched from the debit entry
                                ...
                                <small class="form-text text-muted">Debit and Credit amounts must match.</small>
                            </div>
                        </div>
                    </div>

                    <div class="card-footer">
                        <?= form_submit('submit', 'Update Contra Voucher', ['class' => 'btn btn-info']) ?>
                        <a href="<?= site_url('contra-vouchers') ?>" class="btn btn-default float-right">Cancel</a>
                    </div>
                    <?= form_close() ?>
                </div>
            </div>
