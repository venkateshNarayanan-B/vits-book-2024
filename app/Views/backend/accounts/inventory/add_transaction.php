                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Enter Inventory Transaction Details</h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <form action="<?= base_url('inventory/transactions/store') ?>" method="POST">
                                <?= csrf_field() ?>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="stock_item_id" class="form-control select2" id="stock_item_id" style="width: 100%;" required>
                                        ...
                                        <?php foreach ($items as $item): ?>
                                            <option value="<?= $item['id'] ?>" <?= old('stock_item_id') == $item['id'] ? 'selected' : '' ?>><?= $item['item_name'] ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="quantity" class="form-control" id="quantity" value="<?= old('quantity') ?>" required step="any">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="transaction_type" class="form-control" id="transaction_type" required>
                                        ...
                                        <option value="Inward" <?= old('transaction_type') == 'Inward' ? 'selected' : '' ?>>Inward</option>
                                        <option value="Outward" <?= old('transaction_type') == 'Outward' ? 'selected' : '' ?>>Outward</option>
                                    </select>
